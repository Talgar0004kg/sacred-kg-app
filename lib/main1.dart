...
void main() async {
  ...
  // 2. Получаем конкретный экземпляр
  final localization = FlutterLocalization.instance;

  // 3. Вызываем проверку инициализации именно у него
  localization.ensureInitialized();

  // 4. Загружаем словари (убираем await, если был)
  localization.init(
    mapLocales: [
      const MapLocale('EN', AppLocale.EN),
      const MapLocale('RU', AppLocale.RU),
      const MapLocale('KG', AppLocale.KG),
    ],
    initLanguageCode: 'RU',
  );
...
// 1. Создаем экземпляр локализации (вне класса)
...
class SacredKgApp extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return MaterialApp.router(
      ...
      // ИСПОЛЬЗУЙТЕ ВАШУ ПЕРЕМЕННУЮ ЗДЕСЬ:
      supportedLocales: _localization.supportedLocales,
      localizationsDelegates: _localization.localizationsDelegates,
      locale: Locale(settings.language.toLowerCase() == 'kg' ? 'ky' : settings.language.toLowerCase()),
    );
  }
}
...
String mockAiReply({
  ...
  final target = place?.title ?? 'this place';
  final voice = character.id == 'apashka'
      ? 'I would approach it gently:'
      : 'From the heritage record:';
  if (lower.contains('rule') ||
      lower.contains('respect') ||
      lower.contains('behav')) {
    return '$voice at $target, keep the visit quiet, do not touch fragile stones or water sources, ask before photographing people, and leave the path cleaner than you found it.';
  }
  if (lower.contains('time') ||
      lower.contains('season') ||
      lower.contains('when')) {
    return '$voice early morning is usually best for comfort, photos, and calm movement. Mountain routes should be checked seasonally and with local guidance.';
  }
  if (lower.contains('route') ||
      lower.contains('get') ||
      lower.contains('road')) {
    return place == null
        ? '$voice choose a region first, then the app can show a simple route note for the selected sacred place.'
        : '$voice ${place.route}';
  }
  if (lower.contains('tradition') ||
    ...
    return place == null
        ? '$voice many Kyrgyz sacred places connect water, stone, mountains, family memory, and respectful silence.'
        : '$voice ${place.culturalNote}';
  }
  return place == null
      ? '$voice ask about rules, history, route, or best time to visit, and I will answer with mock local guidance.'
      ...
      body: Center(
        child: Column(
          ...
          children: [
            ...
            const Text('Sacred routes of Kyrgyzstan'),
        ...
        child: Padding(
          ...
          child: Column(
            children: [
              ...
              const SizedBox(height: 24),
              Text(
                'Sacred places, respectful routes, living stories.',
...
class _LoginScreenState extends ConsumerState<LoginScreen> {
  ...
  Widget build(BuildContext context) {
    return AuthScaffold(
      title: 'Welcome back',
      subtitle: 'Use any non-empty email and password for the mock session.',
      children: [
        TextField(
          controller: email,
...
class _RegisterScreenState extends ConsumerState<RegisterScreen> {
  ...
  Widget build(BuildContext context) {
    return AuthScaffold(
      ...
      subtitle: 'A local profile is enough for the prototype.',
      children: [
        TextField(
          controller: name,
...
class HomeScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final quickActions = [
      ('Seven regions', 'Choose a route', Icons.public, '/map', AppTheme.leaf),
      (
        'Ask Apashka',
        'Rules and stories',
        Icons.auto_awesome,
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 16),
                  const _HomeSectionHeader(title: 'More to explore'),
                ],
              ),
            ),
          ),
        ...
        child: Stack(
          ...
          children: [
            ...
            Positioned(
              ...
              child: Row(
                children: [
                  const _HeroBadge(label: 'Tunduk route'),
...
class _CultureStoryRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final items = [
      ('Tunduk', 'shared sky', Icons.circle_outlined, AppTheme.rose),
      ('Shyrdak', 'ornament paths', Icons.diamond_outlined, AppTheme.gold),
      ('Komuz', 'living memory', Icons.music_note, AppTheme.leaf),
    ];
    return Row(
      children: [
    ...
    return AppScaffold(
      ...
      child: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          const _DetailSectionTitle(title: 'Regional routes'),
          const SizedBox(height: 10),
          ...
          children: [
            ...
            Positioned(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const _HeroBadge(label: 'Kyrgyzstan regions'),
                  const SizedBox(height: 10),
                  Text(
                    'Choose a sacred direction',
                    maxLines: 2,
                    overflow: TextOverflow.ellipsis,
                    style: Theme.of(context).textTheme.headlineSmall?.copyWith(
                      color: Colors.white,
...
class _RegionStatsRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final petroglyphCount = MockData.places
        ...
        .length;
    final stats = [
      ('7', 'regions', Icons.public, AppTheme.leaf),
      (
      ...
      ('$petroglyphCount', 'petroglyphs', Icons.history_edu, AppTheme.rose),
    ];
    return Row(
      children: [
...
class _RegionRouteCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final featured = places.isEmpty ? 'No places yet' : places.first.title;
    return InkWell(
      borderRadius: BorderRadius.circular(8),
      onTap: () => context.push('/region/${region.id}'),
      child: Container(
...
class _CatalogScreenState extends ConsumerState<CatalogScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return AppScaffold(
      selectedIndex: 2,
      title: regionName == null ? 'Sacred catalog' : '$regionName places',
      child: ListView(
        padding: const EdgeInsets.fromLTRB(16, 12, 16, 24),
        children: [
          ...
          const SizedBox(height: 18),
          _DetailSectionTitle(
            title: '${places.length} sacred places',
            action: sortByRating ? 'Top rated' : 'Popular',
          ),
          const SizedBox(height: 10),
          if (places.isEmpty)
            const EmptyView(message: 'No matching places yet.')
          else
          ...
          children: [
            ...
            Positioned(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  _HeroBadge(label: regionName ?? 'All Kyrgyzstan'),
                  const SizedBox(height: 10),
                  Text(
                    'Sacred catalog',
                    style: Theme.of(context).textTheme.headlineSmall?.copyWith(
                  ...
                  const SizedBox(height: 5),
                  Text(
                    '$resultCount places with stories, rules, routes, and local mock reviews.',
                    maxLines: 2,
                    overflow: TextOverflow.ellipsis,
                    style: Theme.of(context).textTheme.bodyMedium?.copyWith(
                      color: Colors.white.withValues(alpha: 0.92),
        ...
        children: [
          ...
          TextField(
            ...
            decoration: InputDecoration(
              ...
              labelText: 'Search sacred places',
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const _DetailSectionTitle(title: 'Route filters'),
          const SizedBox(height: 8),
    ...
    return AppScaffold(
      ...
      child: ListView(
        ...
        children: [
          ...
          SectionBlock(title: 'Route', text: place.route, icon: Icons.route),
          const SizedBox(height: 18),
          const _DetailSectionTitle(title: 'Place palette'),
          const SizedBox(height: 10),
          _PlacePaletteStrip(place: place),
          const SizedBox(height: 20),
          const _DetailSectionTitle(title: 'Visitor voices'),
          const SizedBox(height: 10),
          if (reviews.isEmpty)
            const EmptyView(message: 'No local reviews for this place yet.')
          else
            ...reviews.map(
              (review) => _ReviewCard(
                review: review,
...
class _PlaceDetailStats extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final stats = [
      ...
      (Icons.category_outlined, place.type.label, 'type', AppTheme.clay),
      (Icons.trending_up, '${place.popularity}', 'popular', AppTheme.rose),
    ];
    return Row(
      children: [
        for (var i = 0; i < stats.length; i++) ...[
...
class _AiAssistantScreenState extends ConsumerState<AiAssistantScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return AppScaffold(
      selectedIndex: 3,
      title: place == null ? 'AI guide' : 'AI guide',
      child: Column(
        children: [
          Expanded(
            child: ListView(
              ...
              children: [
                ...
                const _KyrgyzOrnamentBand(compact: true),
                const SizedBox(height: 16),
                const _DetailSectionTitle(title: 'Choose your guide'),
                const SizedBox(height: 10),
                Row(
                ...
                const SizedBox(height: 18),
                const _DetailSectionTitle(title: 'Conversation'),
                const SizedBox(height: 10),
            ...
            Positioned(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 10),
                  Text(
                    place == null ? 'Ask a cultural guide' : place!.title,
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis,
                    style: Theme.of(context).textTheme.headlineSmall?.copyWith(
                      color: Colors.white,
            ...
            Expanded(
              child: TextField(
                ...
                decoration: const InputDecoration(
                  hintText: 'Ask about rules, route, or traditions',
                ),
      ...
      child: ListView(
        ...
        children: [
          TextField(
            ...
            maxLines: 8,
            decoration: const InputDecoration(
              labelText: 'Share a review, tip, issue, or experience',
            ),
          ),
          const SizedBox(height: 12),
          ...
          DropdownButtonFormField<String?>(
            ...
            decoration: const InputDecoration(labelText: 'Related place'),
            items: [
          ...
          FilledButton(
            ...
            child: const Text('Publish locally'),
          ),
        ],
      ),
    );
    ...
    return AppScaffold(
      ...
      child: ListView(
        ...
        children: [
          ...
          const _KyrgyzOrnamentBand(compact: true),
          const SizedBox(height: 16),
          const _DetailSectionTitle(title: 'Plan your visit'),
          const SizedBox(height: 10),
          _BookingFormCard(
            title: '1. Sacred place',
            subtitle: 'Choose where the visit request should go.',
            icon: Icons.place_outlined,
            color: AppTheme.leaf,
            ...
            onPressed: confirmed
                ? () {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text('Visit request saved locally.'),
                      ),
                    );
                  }
                : null,
          ...
          const SizedBox(height: 24),
          const _DetailSectionTitle(title: 'My visit requests'),
          const SizedBox(height: 10),
          ...bookings.map(
          ...
          children: [
            ...
            Positioned(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const _HeroBadge(label: 'Respectful visit'),
                  const SizedBox(height: 10),
                  Text(
                    place.title,
                    maxLines: 1,
...
class _BookingPeopleCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return _BookingFormCard(
      title: '3. Visitors',
      subtitle: 'Keep the group size clear for local planning.',
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 4),
                  Text(
                    '${DateFormat('MMM d, y').format(booking.date)} - ${booking.peopleCount} people',
                  ),
        ...
        children: [
          ...
          DropdownButtonFormField<String>(
            ...
            items: const [
              ...
              DropdownMenuItem(value: 'KG', child: Text('Kyrgyz')),
            ],
            onChanged: (value) =>
                ref.read(settingsProvider).setLanguage(value ?? 'EN'),
          ),
          const SizedBox(height: 8),
          SwitchListTile(
            value: settings.musicEnabled,
            title: const Text('Music'),
            subtitle: const Text('Mock setting only'),
      ...
      child: ListView(
        ...
        children: [
          ...
          if (favorites.isEmpty)
            const EmptyView(message: 'Favorite places will appear here.')
          else
          ...
          _HomeToolRow(
            ...
            subtitle: 'Visitor tips and local updates',
            ...
            children: [
              ...
              if (widget.showTitle)
                Positioned(
                  ...
                  bottom: 16,
                  child: Text(
                    widget.place?.title ?? 'Kyrgyz sacred routes',
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis,
                    style: Theme.of(context).textTheme.titleLarge?.copyWith(
                      color: Colors.white,
